...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_path', type=str, default='./debug/')
    ...
    if args.mode == 'pair':
        ...
        for split_id in range(num_splits):
            ...
            with open(args.out_path + 'tensors-%d.pkl' % split_id, 'wb') as f:
                pickle.dump(sub_data, f, pickle.HIGHEST_PROTOCOL)
        '''

        num_splits = len(data) // (args.batch_size * 1000)
        ...
        for split_id in tqdm(range(num_splits)):
            ...
            batches = [sub_data[i : i + args.batch_size] for i in range(0, len(sub_data), args.batch_size)]
            func = partial(tensorize, vocab = args.vocab)
            with Pool(args.ncpu) as p:
                out_data = p.map(func, batches)

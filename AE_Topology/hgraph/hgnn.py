...
class HierVAE(nn.Module):
    ...
    # YY: z --> G
    def decode(self, z):
        root_vecs, root_kl = self.rsample(z, self.R_mean, self.R_var, perturb=False)
        smiles = self.decoder.decode((root_vecs, root_vecs, root_vecs), greedy=True, max_decode_step=150)
        return smiles
    
    # global-local contrastive loss
    def cl_loss(self, global_vecs, local_vecs, idxs):
        # idxs = torch.tensor([np.random.choice(idx_plus) + idx_start for idx_start, idx_plus in idxs]).to(global_vecs.device)
        # local_vecs = local_vecs[idxs]

        # drop 20% nodes and mean pooling
        local_vecs = torch.cat( [local_vecs[np.random.choice(idx_plus, int(idx_plus*0.8), replace=False) + idx_start].mean(dim=0).unsqueeze(0) if idx_plus > 1
                                    else local_vecs[np.random.choice(idx_plus) + idx_start].unsqueeze(0) for idx_start, idx_plus in idxs], dim=0 )

        x1 = global_vecs # shape = (batch_size, hidden_size)
        x2 = local_vecs # shape = (batch_size, hidden_size)
    ...
    def forward_with_gssl(self, graphs, tensors, orders, beta, perturb_z=True):
        tree_tensors, graph_tensors = tensors = make_cuda(tensors)

        root_vecs, tree_vecs, _, graph_vecs = self.encoder(tree_tensors, graph_tensors)

        # contrastive loss
        loss_cl = self.cl_loss(root_vecs, graph_vecs, graph_tensors[-1])

        root_vecs, root_kl = self.rsample(root_vecs, self.R_mean, self.R_var, perturb_z)
        kl_div = root_kl

        loss, wacc, iacc, tacc, sacc = self.decoder((root_vecs, root_vecs, root_vecs), graphs, tensors, orders)
        # print(loss, loss_cl, kl_div)
        return loss + beta * kl_div + 1e-3 * loss_cl, kl_div.item(), wacc, iacc, tacc, sacc

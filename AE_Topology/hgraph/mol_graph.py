...
class MolGraph(object):
    ...
    def tensorize(mol_batch, vocab, avocab):
        '''
        # YY
        for smi in mol_batch:
            try:
                MolGraph.tensorize_graph([MolGraph(smi)], vocab)
            except:
                print(smi)
                1 / 0
        '''
        ...
        '''
        # YY
        for node, attr in mol_batch[0].mol_tree.nodes(data=True):
            smiles = attr['smiles']
        ...
        for x in mol_batch:
            try:
                MolGraph.tensorize_graph([copy.deepcopy(x).mol_tree for x in mol_batch], vocab)
                _mol_batch.append(x)
            except:
                pass
        mol_batch = _mol_batch
        # YY: handle ood vocab
        '''

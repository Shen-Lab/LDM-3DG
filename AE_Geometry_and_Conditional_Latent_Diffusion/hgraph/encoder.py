...
class HierMPNEncoder(nn.Module):
    ...
    def embed_inter(self, tree_tensors, hatom):
        ...
        pos_vecs = self.E_pos.to(fnode.device).index_select(0, fmess[:, 2])
    ...
    def embed_graph(self, graph_tensors):
        ...
        hnode = self.E_a.to(fnode.device).index_select(index=fnode, dim=0)
        fmess1 = hnode.index_select(index=fmess[:, 0], dim=0)
        fmess2 = self.E_b.to(fnode.device).index_select(index=fmess[:, 2], dim=0)
        fpos = self.E_apos.to(fnode.device).index_select(index=fmess[:, 3], dim=0)
...
class IncHierMPNEncoder(HierMPNEncoder):
    ...
    def embed_sub_tree(self, tree_tensors, hinput, subtree, is_inter_layer):
        ...
        if len(submess) == 0:
            ...
            pos_vecs = self.E_pos.to(fmess.device).index_select(0, fmess[:, 2])
            hmess = torch.cat( [hmess, pos_vecs], dim=-1 ) 
        return hnode, hmess, agraph, bgraph 

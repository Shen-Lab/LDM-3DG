...
import numpy as np
from dmcg_utils.utils.utils import get_random_rotation_3d, clip_norm
import pdb
...
class GNN(nn.Module):
    def __init__(
        ...
        self.global_init_mlp = nn.Sequential( nn.Linear(latent_size + 500, latent_size),
                                              nn.ReLU(),
        ...
        self.cycle = cycle
        self.ang_lam = ang_lam
        self.bond_lam = bond_lam
    ...
    def forward(self, batch, emb_2dand3d, sample=False, randomness=True):
        ...
        u_embed = torch.cat([u_embed, emb_2dand3d], dim=1)
        u_embed = self.global_init_mlp(u_embed)
        ...
        if randomness:
            ...
            cur_pos = x_embed.new_zeros((x_embed.size(0), 3))
        '''

        # added by YY
        cur_pos = batch.pos_init
        # added by YY
        ...
        if not sample:
            ...
            edge_attr = edge_attr_embed
            u = u_embed
            cur_pos = self.move2origin(batch.pos_init, batch)
            ...
            # z = torch.zeros(u_embed.shape).to(x.device)
        ...
        cur_pos = batch.pos_init # pos_list[-1]
        pos_list = []
    ...
    def compute_loss(self, pos_list, extra_output, batch):
        loss_dict = {}
        loss = 0
        ...
        '''
        new_idx = torch.arange(pos.shape[0]).to(pos.device)
        ...
        loss_tmp, _ = self.alignment_loss(
            pos, pos_list[-1].index_select(0, new_idx), batch, clamp=self.clamp_dist
        )
        loss = loss + loss_tmp
        ...
        if self.aux_loss > 0:
            for i in range(len(pos_list) - 1):
                loss_tmp, _ = self.alignment_loss(
                    pos, pos_list[i].index_select(0, new_idx), batch, clamp=self.clamp_dist
                )
                loss = loss + loss_tmp * (self.aux_loss if i < len(pos_list) - self.cycle else 1.0)
                loss_dict[f"loss_pos_{i}"] = loss_tmp

        if self.ang_lam > 0 or self.bond_lam > 0:
            ...
            loss = loss + self.bond_lam * bond_loss + self.ang_lam * angle_loss
    ...
    def get_score(self, batch, emb_2dand3d, pos, args):

...
from geometric_gnn_dojo_utils.modules.blocks import (
    EquivariantProductBasisBlock,
    RadialEmbeddingBlock,
)
from geometric_gnn_dojo_utils.modules.irreps_tools import reshape_irreps

from geometric_gnn_dojo_utils.egnn_layers import MPNNLayer, EGNNLayer
from geometric_gnn_dojo_utils.tfn_layers import TensorProductConvLayer
import geometric_gnn_dojo_utils.gvp_layers as gvp
import pdb
...
class MACEModel(torch.nn.Module):
    ...
    def forward(self, batch, norm=False):
        ...
        # YY: radius_edge_index
        vectors = batch.pos[batch.radius_edge_index[0]] - batch.pos[batch.radius_edge_index[1]]  # [n_edges, 3]
        # YY: radius_edge_index
        ...
        for conv, reshape, prod in zip(self.convs, self.reshapes, self.prods):
            ...
            # YY: radius_edge_index
            h_update = conv(h, batch.radius_edge_index, edge_attrs, edge_feats)
            # YY: radius_edge_index
        ...
        if not norm:
            return self.tanh( self.pred(out) )  # (batch_size, out_dim)
        else:
            # normalize to [-1, 1]
            emb = self.pred(out) # (bs, dim)
            emb = (emb - emb.min(dim=1)[0].reshape(-1, 1)) / (emb.max(dim=1)[0].reshape(-1, 1) - emb.min(dim=1)[0].reshape(-1, 1) + 1e-10) * 2 - 1
            return emb
...
class EGNNModel(torch.nn.Module):
    ...
    def forward(self, x, pos, edge_index, batch, tanh=False, with_local=False):
        ...
        if not with_local:
            return pos, out  # (batch_size, out_dim)
        else:
            return pos, out, self.pred(h)

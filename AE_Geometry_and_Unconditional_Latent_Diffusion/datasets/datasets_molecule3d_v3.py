...
class Molecule3D(InMemoryDataset):
    def __init__(
        ...
        torch.save(mol_list, 'processed_data_2d_to_3d/mol_train.pt')
        print(len(mol_list), len(self.data))

        mol_list = torch.load('./processed_data_2d_to_3d/mol_train.pt')
        from multiprocessing import Pool
        batch_size = 128
        batch_list = []
        for idx, mol in enumerate(tqdm(mol_list)):
            AllChem.EmbedMolecule(mol)
        torch.save(mol_list, 'processed_data_2d_to_3d/mol_pos_init_train.pt')

        d1 = torch.load('./processed_data_2d_to_3d/mol_train.pt')[0].GetConformer().GetPositions()
        d2 = torch.load('./processed_data_2d_to_3d/mol_pos_init_train.pt')[0].GetConformer().GetPositions()
        print(d1 - d2)
        assert False
        '''

        self.mol_list = torch.load('./processed_data_2d_to_3d/mol_pos_init_train.pt')
    ...
    def get_init2d_pos(self, mol):
        '''
        data_x, data_edge_index, data_edge_attr = data.x[:, :118], data.edge_index, data.edge_attr
        mol = Chem.RWMol()

        # atoms
        atom_features = data_x.argmax(dim=1).cpu().numpy() + 1
        # print(atom_features)
        num_atoms = atom_features.shape[0]
        for i in range(num_atoms):
            atom = Chem.Atom(atom_features[i].item())
            mol.AddAtom(atom)

        # bonds
        allowable_bond_list = [Chem.rdchem.BondType.SINGLE, Chem.rdchem.BondType.DOUBLE, Chem.rdchem.BondType.TRIPLE, Chem.rdchem.BondType.AROMATIC]
        edge_index = data_edge_index.cpu().numpy()
        edge_attr = data_edge_attr.argmax(dim=1).cpu().numpy()
        # print(edge_index)
        # print(edge_attr)
        ...
        for j in range(0, num_bonds, 2):
            ...
            bond_type = allowable_bond_list[edge_attr[j]]
            mol.AddBond(begin_idx, end_idx, bond_type)
        '''

        positions = torch.tensor( mol.GetConformer().GetPositions(), dtype=torch.float32 )
        edge_index_pos = radius_graph(positions, r=5.0, loop=False)
        return positions, edge_index_pos
    ...
    def get(self, idx):
        ...
        # get init positions
        try:
            data.pos_init, data.edge_index_pos_init = self.get_init2d_pos(self.mol_list[idx])
        except:
            '''
            with open('mol_idx.txt', 'a+') as f:
                f.write(str(idx) + '\n')
            '''
            data.pos_init = torch.randn((data.x.shape[0], 3))
            adj = 1 - torch.eye(data.x.shape[0])
            data.edge_index_pos_init, _ = tgeom.utils.dense_to_sparse(adj)

...
class Model(pl.LightningModule):
    ...
    def loss_cl(self, global_vecs, local_vecs, batch_n_nodes):
        idx = []
        count = 0
        vecs = []
        for n_nodes in batch_n_nodes:
            # idx.append(count + np.random.choice(n_nodes.item(), int(n_nodes.item()*0.8), replace=False))
            # random dropping 20% nodes and mean pooling
            if n_nodes.item() > 1:
                vecs.append( local_vecs[count + np.random.choice(n_nodes.item(), int(n_nodes.item()*0.8), replace=False)].mean(dim=0).unsqueeze(0) )
            else:
                vecs.append( local_vecs[count + np.random.choice(n_nodes.item())].unsqueeze(0) )
            count += n_nodes.item()
        # idx = torch.LongTensor(idx).to(global_vecs.device)
        # local_vecs = local_vecs[idx]
        local_vecs = torch.cat(vecs, dim=0)
    ...
    def training_step(self, batch, batch_idx):
        ...
        _, emb_3d, emb_3d_local = self.encoder_3d(batch.x, batch.pos, batch.radius_edge_index, batch.batch, True, True)
        
        loss_cl = self.loss_cl(emb_3d, emb_3d_local, batch.n_nodes)
        ...
        loss += loss_neg_contrast * 0.1 + + 1e-3 * loss_cl

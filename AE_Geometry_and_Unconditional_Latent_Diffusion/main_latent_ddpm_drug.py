...
if __name__ == '__main__':
    ...
    data = torch.load( os.path.join(args.data_dir, 'emb.pt') )
    print(data['emb_2d'].shape)

    idx = np.load(os.path.join(args.data_dir, 'geom_permutation.npy'))
    num_mol = idx.shape[0]
    val_index = int(num_mol * 0.1)
    test_index = val_index + int(num_mol * 0.1)
    idx_val, idx_test, idx_train = np.split(idx, [val_index, test_index])
    print(idx.shape, idx_train.shape, idx_val.shape, idx_test.shape)

    dataset_train = torch.utils.data.TensorDataset(torch.cat([data['emb_2d'][idx_train], data['emb_3d'][idx_train]], axis=1)) # shape = (N, 250+250)
    dataloader_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=4)

    dataset_val = torch.utils.data.TensorDataset(torch.cat([data['emb_2d'][idx_val], data['emb_3d'][idx_val]], axis=1)) # shape = (N, 250+250)
    dataloader_val = torch.utils.data.DataLoader(dataset_val, batch_size=args.batch_size*8, shuffle=False, num_workers=4)

    dataset_test = torch.utils.data.TensorDataset(torch.cat([data['emb_2d'][idx_test], data['emb_3d'][idx_test]], axis=1)) # shape = (N, 250+250)

...
class Vocab(object):
    ...
    def __getitem__(self, smiles):
        ...
        try: return self.vmap[smiles]
            ...
            torch.save({}, 'common_atoms/' + smiles[0] + '_' + str(smiles[1]) + '.pt')
            return 0
        '''
...
class PairVocab(object):
    ...
    def get_mask(self, cls_idx):
        return self.mask.to(cls_idx.device).index_select(index=cls_idx, dim=0)
...
# more atoms
for fn in os.listdir('/scratch/user/yuning.you/project/graph_latent_diffusion/hgraph2graph/common_atoms/'):
    if not '.pt' in fn: continue
    atom, charge = fn[:-3].split('_')
    charge = int(charge)
    COMMON_ATOMS.append((atom, charge))

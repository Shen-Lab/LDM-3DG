...
class HierMPNDecoder(nn.Module):
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        hinter = HTuple(
            ...
            emask = self.itensor.new_zeros(inter_tensors[1].size(0)).to(src_root_vecs.device)
        )
        hgraph = HTuple(
            mess = self.rnn_cell.get_init_state(graph_tensors[1]),
            vmask = self.itensor.new_zeros(graph_tensors[0].size(0)).to(src_root_vecs.device),
            emask = self.itensor.new_zeros(graph_tensors[1].size(0)).to(src_root_vecs.device)
        )
    ...
    def enum_attach(self, hgraph, cands, icls, nth_child):
        cands = self.itensor.new_tensor(cands).to(self.topoNN[0].weight.device)
        icls_vecs = self.itensor.new_tensor(icls * len(cands)).to(self.topoNN[0].weight.device)
        icls_vecs = self.E_assm( icls_vecs )

        nth_child = self.itensor.new_tensor([nth_child] * len(cands.view(-1))).to(self.topoNN[0].weight.device)
        order_vecs = self.E_order.to(self.topoNN[0].weight.device).index_select(0, nth_child)
    ...
    def decode(self, src_mol_vecs, greedy=True, max_decode_step=100, beam=5):
        ...
        init_vecs = src_root_vecs if self.latent_size == self.hidden_size else self.W_root(src_root_vecs)
        batch_idx = self.itensor.new_tensor(range(batch_size)).to(self.topoNN[0].weight.device)

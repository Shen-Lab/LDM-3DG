...
class DatasetFromFolder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        idx_fn = idx // 1000
        idx_batch = idx % 1000
        if not self.load_fn == self.data_files[idx_fn]:
            self.load_fn = self.data_files[idx_fn]
            fn = os.path.join(self.data_folder, self.load_fn)
            with open(fn, 'rb') as f:
                self.load_batches = pickle.load(f)
                random.shuffle(self.load_batches)

        if idx_batch >= len(self.load_batches):
            idx_batch = np.random.choice(len(self.load_batches), 1)

        batch = self.load_batches[idx_batch][:3]
        return batch
